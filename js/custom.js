	...
	function viewalldept(){
		$.ajax({
			...
			success:function(data){
				...
				for (var i = 0, len = data.length; i < len; i++){
					table +='<tr>';
					table +='<td>'+(i+1)+'</td><td id="td_id_'+data[i].dep_id+'"><span id="Ed_id_'+data[i].dep_id+'">'+data[i].dep_name+'</span><button class="btn btn-outline-danger btn-sm m-btn m-btn--icon m-btn--pill" disabled="disabled" onclick="delete_dep('+data[i].dep_id+')" style="float:right;">  <i class="la la-trash"></i></button><button class="btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--pill" id="atag_id_'+data[i].dep_id+'" data="Edit" style="float:right;margin-right: 2px;" onclick="edit_dept('+data[i].dep_id+',this)">Edit</button> </td>'; 
					//console.log(data[i].name);
					table +='<tr>';
				}
				$('#deptDiv').html(table);
...
function delete_dep(dep_id){
	...
	if(result){
//		alert("Are you sure to delete this department");
			event.preventDefault();
			$.ajax({
						url:'./delete_depts',
						data:{dep_id},
...
//							console.log(data);
							if(data.status == 1){
								...
								$.notify({
									title: '<strong>Error!</strong>',
									message:"Dept couldn't be deleted!"
								},{
									type: 'danger',
									z_index: 10000,
								});
